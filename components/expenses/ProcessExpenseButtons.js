...
const ProcessExpenseButtons = ({ expense, collective, permissions, buttonProps }) => {
  ...
  const getButtonProps = (action, callDefaultProcessExpense = true) => {
    ...
    return {
      ...
      onClick: () => {
        ...
        if (callDefaultProcessExpense) {
          return processExpense({ variables: { action } });
        }
      },
    };
  };
  ...
  return (
    <React.Fragment>
      ...
      {permissions.canPay && (
        <PayExpenseButton {...getButtonProps('PAY', false)} expense={expense} collective={collective} />
      )}
